...
class Core:
    ...
    def prune_state_save(me):
        """
        method will look through the different saved state instantiations held
        in myglobals.Variables.current_assignments and remove them if they are
        pointing to ships that no longer exist

        :return:
        """

        ships = me.get_ships().id

        for assignment in myglobals.Variables.current_assignments:
            if assignment.id != -1 and not assignment.id in ships:
                # we've got a dead ship walkin' heah
                del myglobals.Variables.current_assignments[assignment.id]
